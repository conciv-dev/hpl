...
import { readJournal } from '@napl/core';
import type { LlmClient } from '@napl/core';
import { emptyMap, recordAttribution, writeMap } from '@napl/core';
...
async function seedPriorGen(): Promise<void> {
  ...
  recordAttribution(map, {
    ...
    files: [{ filePath: '.napl/src/typescript/greeting.ts', hash: contentHash(V1) }],
  });
  ...
  await writeFile(
    join(root, '.napl', 'attribution', 'greeting.yaml'),
    'module: greeting\ntarget: typescript\nentries:\n  - promptLines: [1, 1]\n    file: greeting.ts\n    lines: [1, 1]\n    note: greet\n',
    'utf8',
  );
}
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'napl-genjournal-'));
...
describe('runGen journal', () => {
  ...
  it('records an incremental entry with a prompt diff and a scoped file patch, and blame reflects the new gen', async () => {
    ...
    const lines: string[] = [];
    await runBlame({ root, file: '.napl/src/typescript/greeting.ts', log: (m) => lines.push(m) });
    expect(lines.join('\n')).toContain('gen #1');
  });
});
