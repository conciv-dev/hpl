...
function blameGen(entries: JournalEntry[], gen: number, log: (message: string) => void): BlameResult {
  ...
  if (entry === undefined) {
    log(`napl blame: no journal entry for gen #${gen}`);
    return { exitCode: 1 };
  }
...
export async function runBlame(options: BlameOptions): Promise<BlameResult> {
  ...
  if (entries.length === 0) {
    log('napl blame: no gen journal found — run `napl gen` to start recording line history.');
    return { exitCode: 1 };
  }
  ...
  if (options.file === undefined) {
    log('napl blame: provide a generated file path, or --gen <n>.');
    return { exitCode: 1 };
  }
  ...
  if (history.length === 0) {
    log(`napl blame: no journal history for ${relPath} — is it a generated file under .napl/src/?`);
    return { exitCode: 1 };
  }

  const abs = join(options.root, relPath);
  if (!existsSync(abs)) {
    log(`napl blame: file not found on disk: ${relPath}`);
    return { exitCode: 1 };
  }
  ...
  if (options.line !== undefined && rows.length === 0) {
    log(`napl blame: line ${options.line} is out of range for ${relPath} (${blamed.length} line(s)).`);
    return { exitCode: 1 };
  }
