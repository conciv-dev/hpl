...
- **`extensions`**: return-type _ownership_ was unstated — the agent returned
...
- **`parse_output`**: the machine layer flagged the "no closing backticks after a
  ...
  actually _stricter_ than the hand-written `find("```")` — the two agree on every
...
- The gen loop (`napl-cli/src/cmd_gen.rs`) refreshes the toolchain-owned workspace
  ...
  crate directory plus the current module — so the member is registered _before_
...
real: a wave-2 crate must build on the _generated_ wave-1 crate, not re-implement
...
each dep section states that the sibling path-dep is _part of the same workspace,
not an external crates.io dependency_, and depending on a sibling by path only
...
`LineRange` a `reverse` match carries _is_ the `LineRange` its `AttributionEntry`
...
build on the _generated_ wave-1 crates by path, expressed entirely in prompt prose,
...
yaml_gen as yaml`, etc.); the cleaner long-run one is to promote the generated
   `.napl/src/rust/` workspace to _be_ `napl-core`'s modules. Start mechanical —
...
- `selfhost/schemas_journal.napl` now pins the exact warning as **behavior prose**:
  ...
  _is_ valid JSON but fails to deserialize/validate takes the
...
of `schemas_journal::JournalEntry` and both crates path-dep the _same_
...
_pure decision logic_ — file classification, description derivation, prompt-diff
...
### The honest finding — this is a _literal_ fixpoint, not a rewrite
...
_could_ differ — but a strong coding agent, handed an unchanged prompt and an
...
This is a _stronger_ statement than the behavioral fixpoint the campaign set out to
...
against the _existing_ code, it does not delete-then-regenerate — so the demonstrated
...
test helper in the _same_ generated workspace. Because the rust target's in-gen gate
...
ordering only works for shape-_preserving_ regens.
...
the extracted pure cores of `napl-cli`). What is _not_ generated, and stays
...
means deleting any _remaining hand-written module bodies whose behavior is now proven
generated_, not this I/O skeleton.

## Deps-enforcement slice — gen-time sibling-dependency gate (2026-07-24)

The fixpoint above proved every module regenerates byte-identically _given a correct
prompt_. This slice closes the gap the fixpoint could not see: a prompt that leaves
...
imperative — _"Add a path dependency on each in your `Cargo.toml`… use their public
API, and do not reimplement their logic or depend on any hand-written crate"_ — leaving
...
The lesson is the reusable one: **a prompt that only _describes_ a sibling's types
invites reimplementation; a prompt must _instruct_ the agent to depend on and use them.**
...
The fixpoint above proved every _pure_ module (all of `napl-core` and the extracted
...
### Batch 2 — the subprocess and state leaves (DONE)

Two modules, each generated from a behavior-prose prompt in `selfhost/cli/`, each
**converged on attempt 1 of 3**, each swapped in behind unchanged call sites,
conformance **48/48 byte-identical** after every swap:

| Generated crate | Replaces (napl-cli surface)                                                                                                                  | Class                        | Deps                                                                        | Gate                                                      |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- | --------------------------------------------------------------------------- | --------------------------------------------------------- |
| `process_run`   | `process::{run_command, run_agent, run_codex, run_custom, llm_complete, program_available, GenLock, RunOutput, is_alive, acquire_gen_lock*}` | subprocess/lock shell (leaf) | —                                                                           | `25` + `51` + all gen/spawn + crate lock round-trip tests |
| `state_io`      | `state::{read_map, write_map, read_journal, append_journal_entry, read_lock, write_lock, default_lock, load_prompt_aliases}`                 | fs state shell               | `fsutil_io`, `schemas_map`, `schemas_lock`, `schemas_journal`, `extensions` | `25` + all gen + crate fs round-trip tests                |

- **`process_run` is the first subprocess self-host.** The generated crate owns
  every `std::process` spawn — the `claude`/`codex`/custom coding-agent runners, the
  `claude` LLM one-shot client, the target test-command runner, the `--version`
  availability probe, and the `kill -0` liveness probe — plus the exclusive-create
  gen lockfile. It returns `std` types (`RunOutput`, `bool`, `GenLock`) or a plain
  `String` message for the error-carrying operations, and the `process.rs` shell maps
  those with `.map_err(CliError::new)`. The byte-exact lock-contention message
  (pinned by `25-lock-contention`) and the spawn-failure messages are produced in
  the generated crate. The schema-coupled engine dispatch (`AgentEngine`,
  `resolve_engine(&AgentConfig)`, `require_engine`, `run_coding_agent`) stays
  hand-written in the shell and calls the generated primitives — so the leaf stays
  `deps: []` and neither `CliError` nor any schema type crosses into it. The four
  lock round-trip tests live in both the generated crate (its behavioral corpus) and
  the shell (a regression net). Two small seams: the generated `GenLock` omits a
  `Debug` derive (not behavior), so the shell's contention test matches on the `Err`
  rather than `unwrap_err()`; and the shell's `acquire_gen_lock` routes through its
  own `acquire_gen_lock_with(.., &process_run::is_alive)` to keep that wrapper live.
- **`state_io` is the first generated cli crate to compose on another generated cli
  crate** (`fsutil_io`) alongside four generated `napl-core` schema crates. It reads
  and writes the map/journal/lock, doing the filesystem I/O over the generated
  parsers and serializers. The load-bearing contract is the error bytes: the crate
  returns the **bare underlying message** as a `String` — the schema parser's error,
  the I/O error's `to_string`, or the one hard-coded `missing .napl/lock.json — run
'napl init' first` — which is exactly what the shell's `From<SchemaError>` /
  `From<io::Error>` glue already produced, so the `state.rs` shell wraps each with
  `.map_err(CliError::new)` and the bytes are identical. The returned
  `NaplMap`/`HlLock`/`JournalEntry` are the same generated types `napl_core::schemas`
  re-exports, so they unify and the callers are untouched (`default_lock` is a bare
  re-export). The swap also dropped the now-unused `fsutil::mkdir_parent` re-export.

Gate results (post-batch-2): conformance **48/48 byte-identical** · equivalence
**231/231** (unchanged — both modules are I/O shells with no pure equivalence
surface) · `cargo test --workspace` **247/247** · clippy **clean** · `napl status`
**41/41 clean**. Escape-hatch list still **empty**. The machine-layer (`.mapl`)
derivation was a non-fatal empty-write for both modules (same class as `clock_fmt`),
which does not affect the swapped source or any gate.

### Batch 3 — the fs-walk and classifier shells (4 of 5 DONE)
...
| ---------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| `paths_walk`     | `paths::find_prompt_files`                                  | `extensions`                                                                         |
| `snapshot_io`    | `snapshot::{snapshot_hashes, snapshot_contents}`            | `hash`, `snapshot_filter`                                                            |
| `statusclass_io` | `statusclass::classify`                                     | `hash`, `schemas_map`, `statusclass_render`                                         |
| `driftdetect_io` | `driftdetect::{classify_file (internal), detect_gen_drift}` | `drift`, `driftdetect_replay`, `hash`, `schemas_journal`, `schemas_map`, `text_diff` |

- **`paths_walk`** owns the prompt-file discovery walk — `IGNORED_DIRS`
  (`node_modules`/`.napl`/`.git`), a missing root directory returning an empty list,
  and a sorted result — converting the shell's `&[String]` extension list into the
  `Option<&[&str]>` its `extensions::is_prompt_file` dep expects. The `paths.rs`
  shell is a bare `pub use` of both `paths_core` and `paths_walk`.
- **`snapshot_io`** walks a tree into an ordered `BTreeMap` of content hashes or full
  contents behind a `SnapshotFilter`, with any `read_dir` error mapping to an empty
  snapshot. The shell wraps its `Result<_, String>` with `.map_err(CliError::new)`
  and keeps re-exporting the pure `snapshot_diff`/`snapshot_filter` cores.
- **`statusclass_io`** classifies one prompt's status in strict order (drift →
  unattributed → never-generated → prompt-stale → clean), with the **frontmatter
  parse deliberately kept in the shell** so the bridged `FrontmatterError` bytes are
  preserved; the shell passes the already-parsed `module` + `targets` in, and only
  I/O read errors cross the seam as a `String`.
- **`driftdetect_io`** owns the generation-drift walk: it reconstructs each
  attributed file's journal baseline, reads the current file off disk, and reports
  `Edited`/`Missing` drift as a `Vec<ModuleDrift>`. The private `classify_file`
  moved into the generated crate and the `driftdetect.rs` shell shrank to a
  `detect_gen_drift` wrapper (`.map_err(CliError::new)`) plus the still-needed
  `pub use driftdetect_replay::reconstruct_file_content` re-export (consumed by
  `healing`). The returned `ModuleDrift`/`JournalEntry`/`NaplMap` are the same
  generated types `napl_core` re-exports, so the `cmd_gen`/`cmd_reconcile` callers
  are untouched.

Gate results (post-batch-3, 4 modules): conformance **48/48 byte-identical** ·
