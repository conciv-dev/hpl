...
| `healing`       |  262 | `healing_core`, `healing_io`      | **swapped** (batch 4): thin adapter over the generated verdict + shell crates                                                                                                                    | **(a)** pure decision + **(b)** fs shell                  | `28`,`29`,`35`                                                           |
...
### Batch 3 — results (4 of 5 DONE; `healing_io` deferred, landed in batch 4)
...
(the `heal_moved_files` LCS-verdict shell) was deferred here for a structural split
decision (extract the pure LCS verdict crate first) and landed in **batch 4** below.
...
### Batch 4 — results (healing, the LCS move-match shell)

`healing.rs` (262 LOC) self-hosted as **two** modules by a decided split: a pure
verdict crate and the fs/journal shell composing it, each converged on **attempt 1
of 3**, swapped in behind the unchanged public surface, conformance **48/48
byte-identical** after the swap. Totals: **47/47 generated modules**, escape-hatch
list still empty.

| Generated crate | Replaces (napl-cli surface)                                       | Class                 | Deps                                                                                                                                        | Gate                                              |
| --------------- | ----------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `healing_core`  | the pure move-match verdict (`lcs_len` + clean/drifted/ambiguous) | (a) pure decision     | `text_diff`                                                                                                                                 | `28`,`29`,`35` + equivalence 8/8                  |
| `healing_io`    | `healing::heal_moved_files`                                        | (b) fs/journal shell  | `healing_core`, `hash`, `schemas_journal`, `schemas_map`, `targets`, `snapshot_io`, `snapshot_filter`, `driftdetect_replay`, `fsutil_io`, `paths_core`, `state_io` | `28`,`29`,`35` + crate fs round-trip tests        |

`healing_core` is the pure straggler the wave kept flagging: `lcs_len` and
`move_verdict(old_path, target, old_hash, baseline, candidates, claimed)` decide a
lost file's fate (`Clean`/`Drifted`/`NoMatch`/`Ambiguous`) with the two ambiguity
messages owned byte-exactly (scenario `35` pins the identical-content form). It
depends only on `text_diff::to_lines`, the byte-exact line split the 50-percent
similarity threshold needs, rather than reimplementing it. Its eight-case
equivalence corpus (byte-exact verdicts vs the hand-written `healing.rs`) is the
first pure-verdict corpus of the shell wave. `healing_io` owns the walk and the
write: it finds the tracked files missing from disk, collects the untracked
candidate pool per target (adapter exclusions via `snapshot_filter` + `snapshot_io`),
reconstructs each lost file's baseline (`driftdetect_replay`), asks `healing_core`
for the verdict, then rewrites the map and appends one `move` journal entry per heal
(`state_io`), relocking a clean move at its new path (`fsutil_io`). The clock seam
crosses as an injected `now: &dyn Fn() -> String` (the `process_run` precedent), so
no hand-written type enters the generated crate; the `healing.rs` shell is a thin
`map_err(CliError::new)` wrapper passing `&crate::clock::now`, and dropping its old
`crate::driftdetect::reconstruct_file_content` consumer let the `driftdetect.rs`
re-export move into that module's tests.

**Gate numbers (post-batch-4):** conformance **48/48 byte-identical** (heal
scenarios `28`/`29`/`35` byte-identical) · equivalence **239/239** (231 + `healing_core`
8) · `cargo test --workspace` **247/247** · clippy **clean** · `napl status`
(selfhost) **47/47 clean** · generated-tree `cargo test` all-ok (`healing_core` 19,
`healing_io` 10) · both crates locked `0444`.
