//! Equivalence gate for the `yaml` module.
//!
//! This is the EXACT unit-test corpus of the hand-written `napl-core` `yaml`
//! module (rust/crates/napl-core/src/yaml.rs), replayed against the
//! NAPL-generated `yaml` crate under selfhost/.napl/src/rust/yaml/. The scalar
//! cases pin byte-exact YAML scalar styling; the document cases pin the byte-exact
//! block output the corpus asserts. The `Attribution`/`Ir`/`Ml` inputs are built
//! from the NAPL-generated sibling schema crates, proving the composition
//! `yaml -> {schemas_attribution, schemas_ir, schemas_ml, schemas_line_range}`.

use schemas_attribution::{Attribution, AttributionEntry};
use schemas_ir::{Ir, IrFunction};
use schemas_line_range::LineRange;
use schemas_ml::{Ml, MlEntry, MlKind};
use yaml::{attribution_to_yaml, ir_to_yaml, ml_to_yaml, to_yaml_document, Yaml};

fn scalar(s: &str) -> String {
    let doc = to_yaml_document(&Yaml::Map(vec![("v".to_string(), Yaml::Str(s.to_string()))]));
    ...
    let ml = Ml {
        ...
        entries: vec![
            ...
            MlEntry {
                ...
                reasoning: "the greeting is already emphatic enough; no edit was needed".to_string(),
                suggestion: None,
            },
        ],
    };
